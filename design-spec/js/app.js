...
const motionAllowed = !globalThis.matchMedia("(prefers-reduced-motion: reduce)").matches;
const pageOrder = [
  "index.html",
  "colours.html",
  "typography.html",
...
const updateScrollMeter = () => {
  ...
  const progress = scrollable > 0 ? (globalThis.scrollY / scrollable) * 100 : 0;
  document.documentElement.style.setProperty("--scroll-progress", `${progress}%`);
};

updateScrollMeter();
globalThis.addEventListener("scroll", updateScrollMeter, { passive: true });
globalThis.addEventListener("resize", updateScrollMeter);
...
if (motionAllowed) {
  document.querySelectorAll('a[href$=".html"], a[href="#sections"]').forEach((link) => {
    ...
    link.addEventListener("click", (event) => {
      ...
      if (target.origin !== globalThis.location.origin && globalThis.location.protocol !== "file:") return;
