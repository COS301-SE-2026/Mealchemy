...
class TestHardFilter:
    ...
    def test_allergen_match_removes_recipe(self, recipe_factory, ingredient_factory, user_state_factory):
    ...
    def test_recent_dislike_removes_recipe(self, recipe_factory, user_state_factory, swipe_factory):
        recipe = recipe_factory(recipe_id = 1, dietary_tags = [])
        dislike = swipe_factory(1, "DISLIKED", datetime.now(timezone.utc) - timedelta(days = 1))
    ...
    def test_exclude_recipe_ids_does_not_affect_other_recipes(self, recipe_factory, user_state_factory):
    ...
    def test_mixed_pool_only_eligible_recipes_survive(self, recipe_factory, ingredient_factory, user_state_factory):
        safe_recipe = recipe_factory(recipe_id = 1, dietary_tags = [])
        allergen_recipe = recipe_factory(recipe_id = 2, ingredients = [ingredient_factory(2, category_id = 11)])
