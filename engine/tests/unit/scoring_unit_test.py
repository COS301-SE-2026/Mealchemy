...
class TestComputeScoreBreakdown:
    def test_pantry_match_reflects_actual_coverage(
    ...
    def test_nutrition_and_freshness_are_neutral_in_phase_a(
    ...
    def test_missing_ids_and_pantry_match_agree_with_each_other(
        self, recipe_factory, ingredient_factory, pantry_entry_factory, user_state_factory
    ):
        recipe = recipe_factory(
            ingredients=[ingredient_factory(1), ingredient_factory(2), ingredient_factory(3)]
        )
...
class TestScoreRecipe:
    def test_dot_product_matches_manual_calculation(self, preference_weights_factory):
        breakdown = ScoreBreakdown(
            pantry_match=0.8, cuisine=0.6, nutrition=0.5, novelty=1.0, freshness=0.2
        )
        weights = preference_weights_factory(
            pantry_match=0.40, cuisine=0.25, nutrition=0.15, novelty=0.10, freshness=0.10
        )
    ...
    def test_all_neutral_signals_with_default_weights_scores_neutral(
        self, preference_weights_factory
    ):
        breakdown = ScoreBreakdown(
    ...
    def test_result_is_within_bounds(self, preference_weights_factory):
        breakdown = ScoreBreakdown(
            pantry_match=1.0, cuisine=1.0, nutrition=1.0, novelty=1.0, freshness=1.0
        )
        weights = preference_weights_factory()
...
class TestBuildRecommendationItem:
    def test_pantry_gap_count_matches_missing_ingredients_length(
        self, recipe_factory, ingredient_factory, pantry_entry_factory, user_state_factory
    ):
        recipe = recipe_factory(
            recipe_id=1,
            cuisine="ITALIAN",
            ingredients=[
                ingredient_factory(1, name="Owned"),
                ingredient_factory(2, name="Missing"),
            ],
        )
        user_state = user_state_factory(pantry=[pantry_entry_factory(1)])
