...
class TestBuildTransparencyCard:
    ...
    def test_returns_the_two_highest_scoring_signals_in_order(self, recipe_factory, user_state_factory):
        breakdown = _breakdown(pantry_match=0.9, cuisine=0.8, nutrition=0.1, novelty=0.1, freshness=0.1)
    ...
    def test_percentage_matches_rounded_score(self, recipe_factory, user_state_factory):
        breakdown = _breakdown(pantry_match=0.873, cuisine=0.2, nutrition=0.1, novelty=0.1, freshness=0.1)
        card = build_transparency_card(recipe_factory(), user_state_factory(), breakdown, seed=1)
    ...
    def test_pantry_match_message_cites_matched_and_total_counts(
        ...
        breakdown = _breakdown(pantry_match=0.5, cuisine=0.0, nutrition=0.0, novelty=0.0, freshness=0.0)
    ...
    def test_cuisine_message_cites_the_recipes_cuisine(self, recipe_factory, user_state_factory):
        ...
        breakdown = _breakdown(pantry_match=0.0, cuisine=0.9, nutrition=0.0, novelty=0.0, freshness=0.0)
    ...
    def test_nutrition_with_no_goals_uses_generic_message(self, recipe_factory, user_state_factory):
        ...
        breakdown = _breakdown(pantry_match=0.0, cuisine=0.0, nutrition=0.5, novelty=0.9, freshness=0.0)
    ...
    def test_freshness_with_no_owned_ingredients_uses_generic_message(self, recipe_factory, user_state_factory):
        user_state = user_state_factory(pantry=[])
        breakdown = _breakdown(pantry_match=0.0, cuisine=0.0, nutrition=0.0, novelty=0.9, freshness=0.5)
