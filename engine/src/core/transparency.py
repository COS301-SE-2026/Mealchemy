...
def _render_pantry_match(
    ...
    template = _choose(rng, MESSAGE_TEMPLATES["pantry_match"][_tier(score)])
...
def _render_cuisine(recipe: CandidatePoolEntry, score: float, rng: random.Random) -> str:
    template = _choose(rng, MESSAGE_TEMPLATES["cuisine"][_tier(score)])
...
def _render_nutrition(recipe: CandidatePoolEntry, user_state: UserState, rng: random.Random) -> str:
    ...
    if detail is None:
        return _choose(rng, MESSAGE_TEMPLATES["nutrition"]["none"])

    goal, actual, _, goal_score = detail

    template = _choose(rng, MESSAGE_TEMPLATES["nutrition"][goal][_tier(goal_score)])
...
def _render_novelty(recipe: CandidatePoolEntry, user_state: UserState, rng: random.Random) -> str:
    ...
    return _choose(rng, MESSAGE_TEMPLATES["novelty"][state])
...
def _render_freshness(recipe: CandidatePoolEntry, user_state: UserState, rng: random.Random) -> str:
    ...
    if detail is None:
        return _choose(rng, MESSAGE_TEMPLATES["freshness"]["none"])

    ingredient_name, urgency = detail
    template = _choose(rng, MESSAGE_TEMPLATES["freshness"][_tier(urgency)])
