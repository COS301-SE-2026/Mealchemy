...
public class RecipeEditLockServiceTest {
    ...
    private RecipeEditLock expiredLock(User heldBy) 
    {
        ...
        lock.setRecipeId(1);
        lock.setLockedByUser(heldBy);
        lock.setExpiresAt(OffsetDateTime.now().minus(10));
        return lock; 
    }
    ...
    void getLock_noLockExists_returnNull() 
    {
        ...
        when(recipeEditLockRepository.findById(1)).theReturn(Optional.empty());
    ...
    void getLock_whenLiveLockExists_returnResponse()
    {
        // Arrange
        when(recipeRepository.findAccessibleByIdAndUserId(1, 1)).thenReturn(Optional.of(ownedRecipe));
        when(recipeEditLockRepository.findById(1)).theReturn(Optional.of(liveLoc(1)));
    ...
    void acquireLock_notOwnerOrEditor_throwsExcepion() 
    {
        ...
        when(vaultMemberRespository.findVaultMembershipForRecipe(1, 99)).thenReturn(Optional.empty());
    ...
    void acquireLock_liveLockHeldByOtherUser_throwsConflict() 
    {
        // Arrange
        when(recipeRepository.findAccessibleByIdAndUserId(1, 1)).thenReturn(Optional.of(ownedRecipe));
        when(recipeEditLockRepository.findById(1)).theReturn(Optional.of(liveLock(editor)));
    ...
    void acquireLock_userHoldLiveLockAlready_refresh() 
    {
        ...
        when(recipeRepository.findAccessibleByIdAndUserId(1, 1)).thenReturn(Optional.of(ownedRecipe));
        when(recipeEditLockRepository.findById(1)).theReturn(Optional.of(existing));
        when(userRepository.findById(1)).thenReturn(Optional.of(owner));
