...
(
    ...
    1,
    'Preheat oven to 220°C. Coat the diced chicken in the beaten egg whites, then toss in the corn starch and 3.5g of salt until evenly coated.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Honey Sriracha Chicken and Broccoli'),
    2,
    'Spray a large nonstick sheet pan with olive oil spray. Arrange the coated chicken in a single layer and bake for 10 minutes.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Honey Sriracha Chicken and Broccoli'),
    3,
    'Flip the chicken pieces and add the broccoli florets to the pan. Drizzle the broccoli with 10ml sesame oil, 2.5g salt, and a pinch of pepper. Bake for a further 10 minutes until the chicken is cooked through.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Honey Sriracha Chicken and Broccoli'),
    4,
    'Whisk together the honey, sriracha sauce, seasoned rice vinegar, and remaining sesame oil in a bowl. Toss the baked chicken in the sauce until fully coated.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Honey Sriracha Chicken and Broccoli'),
    5,
    'Divide the cooked brown rice between 8 meal prep bowls. Top with the sauced chicken and roasted broccoli. Garnish with sliced scallions and black and white sesame seeds.'
);
...
(
    ...
    1,
    'Bring a large pot of well-salted water to a boil. Cook the cavatappi pasta according to package instructions, slightly past al dente. Drain, toss with a drizzle of olive oil, and spread on a plate or tray to cool to room temperature.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Caprese Pasta Salad'),
    2,
    'In a large bowl, whisk together the 80ml extra-virgin olive oil, lemon juice, balsamic vinegar, grated pecorino, garlic, salt, and a generous grind of black pepper until combined.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Caprese Pasta Salad'),
    3,
    'Add the cooled pasta to the bowl and toss to coat in the dressing. Add the halved cherry tomatoes, mozzarella balls, red onion, and torn basil. Toss again. Gently fold in the shaved pecorino to finish.'
),
(
    (SELECT recipe_id FROM recipes WHERE title = 'Caprese Pasta Salad'),
    4,
    'Taste and season with additional salt and pepper as needed. Garnish with extra torn basil leaves and serve immediately, or refrigerate for up to two days.'
);
